public class SphereRenderer extends Renderer
{
    ...
    {
        Vector3 centerWorld = Vector3.add(this.sceneObject.transform.position(), this.center);
        double radiusWorld = (this.sceneObject.transform.scale().magnitude() * this.radius);
        double a = Vector3.dot(ray.direction, ray.direction);
        double b = (2*(Vector3.dot(Vector3.sub(ray.origin, centerWorld), ray.direction)));
        double c = Vector3.dot(Vector3.sub(ray.origin, centerWorld), Vector3.sub(ray.origin, centerWorld)) - (radiusWorld*radiusWorld);
        double d = (b*b)-(4*a*c);
        double t;
        if(d >= 0.0)
        {
            t = ((-b - Math.sqrt(d))/(2*a));
            if(t > (Renderer.discriminant))
            {
